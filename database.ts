...
let yearAvailable2017DB: yearAvailable[] = [
...
let yearAvailable2016DB :yearAvailable[] = [
...
const request = indexedDB.open("workersDB",version);
...
request.onupgradeneeded = e => {
    ...
    if (!workersDB.objectStoreNames.contains("teamStore")){
        const teamsObjs = workersDB.createObjectStore("teamStore", {keyPath:"teamName"});
    }
    
    
    if (!workersDB.objectStoreNames.contains("coreStore2016")){
        const coreObjs = workersDB.createObjectStore("coreStore2016", {keyPath:"Id"});
    ...
    if (!workersDB.objectStoreNames.contains("coreStore2017")){
        const coreObjs = workersDB.createObjectStore("coreStore2017", {keyPath:"Id"});
        coreObjs.createIndex("Team" , "Team", {unique : false});
        coreObjs.createIndex("WorkerId" ,"WorkerId", {unique : false});
    }
...
function createObjs(objsName:string, data :any){

    const checkObjs = readAllDB(objsName);

    checkObjs.onsuccess = e =>{
        if(checkObjs.result.length == 0){
            const writeDB = workersDB.transaction(objsName,"readwrite");
            writeDB.onerror = e => alert("error! ${writeDB.error}");

            const objsData = writeDB.objectStore(objsName);
...
function readIndex(objs, index, key){
    ...
    const request = myIndex.getAll(key);
...
function updateData(objs,input){
    ...
    const objsDB = transaction.objectStore(objs);  
    const request = objsDB.put(input);
    return request;
}
